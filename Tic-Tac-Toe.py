...
def turn_p1(new_board):  # ask where player 1 would like to play, it also checks if someone has played in that
    # spot yet
    x = int(input('player 1, give position'))  # asks for position

    if player(new_board, x) == True:  # if the space is open you can play there
        new_board[position(x)] = 'x'  # change the desired position to X
        new_board = (' '.join(map(str, new_board)))  # turns the list into a string
        return new_board
    elif player(new_board, x) == False:
...
def turn_p2(new_board):  # ask where player 2 would like to play, it also checks if someone has played in that spot yet
    ...
    if player(new_board, x):  # if the space is open you can play there
        ...
        new_board = (' '.join(map(str, new_board)))  # turns the list into a string
        return new_board
    elif player(new_board, x) == False:  # if the desired spot is taken it will ask for a new position
        print('you can not play there, go again')
        x = int(input('give new position'))
...
def check_win(new_board, end):  # function that checks to see if either player has won
...
def board_full(new_board,end):

    new_board = new_board.split(' ')
    if new_board[2] and new_board[3] and new_board[4] and new_board[6] and new_board[7] and new_board[8] and new_board[10] and new_board[11] and new_board[12] != '-':
        print('inside at 197')
        end = True
        new_board = (' '.join(map(str, new_board)))
        print(new_board)
...
def main():
    try:
        ...
        game = 0  # loop starts
        end = False  # variable for ending the game if someone wins
        new_board = " \n - - - \n - - - \n - - -"  # original board
        while game < 5:  # while loop that counts how many moves have been made
            ...
            if check_win(new_board, end):  # calls the function that checks if someone has won
                wait = input('press enter to restart')
                if wait == '':
                    return main()
                else:
                    break
            if board_full(new_board,end):
                new_board = new_board.split(' ')
                wait = input('press enter to restart or press any key then enter to end')
                if wait == '':
                    return main()
                else:
                    break
